  ...
  const handleCopy = async () => {
    try {
      if (navigator.clipboard) {
        ...
        console.error("Clipboard API không được hỗ trợ trong trình duyệt này.");
      }
    } catch (err) {
      console.error("Lỗi khi sao chép:", err);
    }
  };
  useOnClickOutside(dialogRef as RefObject<HTMLDivElement>, handleCloseDialog);
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent className="w-56 !bg-primary-gray">
          <DropdownMenuLabel>
            <p className="self-center text-second-gray text-xs font-semibold ">
              <span className="text-[10px] mr-1">
                {dayjs(String(message.createdAt)).format("H:mm")}
              </span>
              {dayjs(String(message.createdAt)).format("DD/MM/YYYY")}
            </p>
          </DropdownMenuLabel>
          ...
          <DropdownMenuGroup>
            ...
            <DropdownMenuSub>
              <DropdownMenuSubTrigger>Bày tỏ cảm xúc</DropdownMenuSubTrigger>
              <DropdownMenuPortal>
                <DropdownMenuSubContent>
                  <DropdownMenuItem className="dark:focus:bg-second-button-background cursor-pointer">
                    Like
