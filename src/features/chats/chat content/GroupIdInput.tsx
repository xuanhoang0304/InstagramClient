import { Images, Mic, Smile } from 'lucide-react';
...
import { cn, handleError, handleMutateWithKey, handleUploadMediaFile } from '@/lib/utils';
...
const GroupIdInput = ({ onSetInputWrapperHeight }: Props) => {
    ...
    const handleCreateMessage = async (data: CreateMessData) => {
        ...
        if (!media.length && !data.text) {
            toast.error(
                "Vui lòng nhập nội dung tin nhắn hoặc chọn lại file phù hợp"
            );
            setLoading(false);
            return;
        }
    ...
    return (
        <>
            ...
            >
                {targetMessage && (
                    <GroupIdInputReplyWrapper></GroupIdInputReplyWrapper>
                )}
                {!!files?.length && (
                    <MessagePreviewMedia
                        files={files}
