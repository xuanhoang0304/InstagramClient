...
import { EllipsisVertical, MessageCircleCode, Mic, MicOff, Video, VideoOff, X } from 'lucide-react';
...
const OfferComponent = () => {
    ...
    const handleCreatePeer = (
        ...
        peer.on("track", (track, remoteStream) => {
            console.log("Received track for user", targetUserId, track.kind);
            const videoEl = peersVideoRef.current[targetUserId];
            if (videoEl) {
                // Cập nhật srcObject với stream mới
    ...
    const handleLeaveCall = useCallback(() => {
        const listUserRoom = userInRoom.filter(
            (u) => u.user._id !== myUser?._id
        );
    ...
    useEffect(() => {
        if (!status || (!allowCamera && !allowMic)) {
            if (stream) {
        ...
        const getUserMedia = async () => {
            try {
                ...
                if (myVideoRef.current && allowCamera && mediaStream) {
                    myVideoRef.current.srcObject = mediaStream;
                    myVideoRef.current
                        .play()
                        .catch((err) =>
                            console.error("Error playing my video:", err)
                        );
                }

                if (mediaStream && peers.length) {
                    peers.forEach(({ peer }) => {
                        const pc = (peer as PeerWithPC)
                            ._pc as RTCPeerConnection;

                        // Thay thế track video
                        const newVideoTrack = allowCamera
                        ...
                        if (videoSender) {
                            if (newVideoTrack) {
                                ...
                                // Xóa track video nếu camera tắt
                                videoSender.replaceTrack(null);
                            }
                        }

                        // Thay thế track audio
                        const newAudioTrack = allowMic
    ...
    }, [allowCamera, allowMic, status, peers]);
    useEffect(() => {
        socket.on(
            "init-call",
        ...
        socket.on(
            "user-change-mic",
            (data: { uId: string; allowMic: boolean }) => {
                const { uId, allowMic } = data;
                if (uId === String(myUser?._id)) return;
                const videoEl = peersVideoRef.current[uId];

                if (videoEl && videoEl.srcObject instanceof MediaStream) {
                    videoEl.srcObject.getAudioTracks().forEach((track) => {
                        track.enabled = allowMic; // Bật/tắt track audio
                    });
                }
                setUserInRoom((prev) =>
                    prev.map((u) =>
                        u.user._id === uId ? { ...u, allowMic } : u
                    )
                );
            }
        );
        ...
        socket.on(
            ...
            async (data: { uId: string; allowCamera: boolean }) => {
                ...
                if (videoEl && videoEl.srcObject instanceof MediaStream) {
                    if (!allowCamera) {
                        // Xóa track video nếu camera bị tắt
                    ...
                    // Không cần thêm track mới ở đây vì peer.on('track') sẽ xử lý
                    const newSteam = await navigator.mediaDevices.getUserMedia({
                        video: allowCamera,
                        audio: true,
                    });
                    videoEl.srcObject = newSteam;
                }
                setUserInRoom((prev) =>
                    prev.map((u) =>
    ...
    }, [myUser?._id, status, userInRoom.length, streamRef.current]);
    useEffect(() => {
                ...
                {status === "isCalling" && (
                    <>
                        ...
                        >
                            ...
                            {userInRoom
                                .filter(
                                    (u) => u.user._id !== String(myUser?._id)
                                )
                                .map((item) => (
                                    <CallVideoItem
                                        key={item.user._id}
