  ...
  const mappingMedia: Record<string, ReactNode> = useMemo(
    () => ({
      ...
      images: (
        ...
        >
          {imageUrls.map((item) => (
            <SwiperSlide key={item.path} className="!h-auto select-none">
        ...
        <div className=" text-center flex justify-center items-center">
          <div>
            ...
            <p className="text-sm text-second-gray mt-3">
              Mỗi bài đăng chỉ được tối đa 1 video hoặc 10 ảnh. Kích thước mỗi
              file ảnh không được lớn hơn 2MB. Kích thước mỗi file video không
              được lớn hơn 5MB.
            </p>
            <button
              onClick={() => onSetStep(0)}
